...
async def add_air_quality_data(req: AirQualityDataReq, email: str = Header(...),
    ...
    for record in req.data:
        try:
            ...
            if not isinstance(record.cf1, (float, int)) or record.cf1 is None:
                raise ValueError(f"cf1 should be a valid number. Found: {record.cf1}")
...
async def get_filtered_metrics(
    ...
    metric: str = Query(..., description="Metric to filter by (FM, cf1, TempC, RH, etc.)"),
    db: AsyncSession = Depends(get_db),
):
   # Validate metric dynamically
    valid_metrics = ["FM", "cf1", "TempC", "RH"]
...
async def get_all_regions_data(
    latitude: float = Query(None, ge=-90.0, le=90.0, description="Latitude must be between -90 and 90"),
    longitude: float = Query(None, ge=-180.0, le=180.0, description="Longitude must be between -180 and 180"),
    db: AsyncSession = Depends(get_db)
):
    logger.info(f"Lat, Long: {latitude}, {longitude}")
   # Base query to fetch distinct data
    query = select(
        AirQualitySiteDB.region,
        AirQualitySiteDB.ID,
    ...
    # Apply filtering only if both latitude & longitude are provided
    if latitude is not None and longitude is not None:
        query = query.where(
            AirQualitySiteDB.Latitude == latitude,
            AirQualitySiteDB.Longitude == longitude
        )
    ...
    response_data = []
    if not rows:
        return response_data  # Return empty list if no data is found
    
    if latitude is None and longitude is None:
        logger.info(f"inside if latitude is None and longitude is None")
        # Organize data into a simple list of objects when no region query is provided
        for row in rows:
            region, site_id, aqs_site_id, latitude, longitude = row
            response_data.append({
        ...
        logger.info(f"inside else case")
        # When region query is provided, return only the IDs
        response_data = [row[1] for row in rows]  # row[1] corresponds to the ID field
